...
use app\admin\Admin;
use app\admin\library\SettingHelper;
class Setting extends Admin
{
	...
	public function qiniu(){
		if(IS_POST){
			...
			\app\library\SettingHelper::set("shuaibo_qiniu", $data);
			\app\library\SettingHelper::set("shuaibo_image_url", "http://" . $domain . "/");
			$this->success("操作成功");
		}
		
		$settings =  \app\library\SettingHelper::get("shuaibo_qiniu",
				['is_open' => 0, 
			'maxSize'=>5*1024*1024,//文件大小
			'rootPath'=>'./',
			'saveName'=>array('uniqid',''),
	...
	public function phone_setting(){
        if(IS_POST){
            !( $tel = I("tel")) && $this->error("请输入电话");
            !( $phone = I("qq")) && $this->error("请输入qq");
           !( $address = I("address")) && $this->error("请输入地址");
           !($weixin = h(I("post.weixin"))) && $this->error("请传入微信二维码");
            $host = $this->image_url;
            $suffix = \app\library\UploadHelper::getInstance()->getThumbSuffix(375, 375);
            $info = ['address' => $address,'qq' => $phone, 'weixin' => $host.$weixin,'tel' => $tel];
          	\app\library\SettingHelper::set("shuaibo_seller_info", $info);
           $this->success("操作成功");
           //1211065934
        }

        $seller_info = \app\library\SettingHelper::get("shuaibo_seller_info",['address' => '杭州市' ,'qq' => '123456789', 'weixin' => '', 'tel' => '']);
	...
	public function jpush_setting(){
		if(IS_POST){
			...
			\app\library\SettingHelper::set("shuaibo_jpush", ['is_open' => $is_open,'app_key' => $app_key,'app_secret' => $app_secret]);
			$this->success("保存成功");
		}
		
		$jpush = \app\library\SettingHelper::get("shuaibo_jpush",['is_open' => 0,'app_key'=> '','app_secret' => '']);
	...
	public function recent_updates(){
		...
		$updates = M("recent_updates")->field("title,type,update_id")->order("date_upd desc")->limit($page->firstRow.",".$page->listRows)->select();
	...
	public function recent_updates_edit(){
		...
		if (IS_POST){
			...
			if ($update_id){
				$data['date_upd'] = time();
				$res = M("recent_updates")->where(['update_id' => $update_id])->save($data);
			}else {
				$data['date_add'] = time();
				$data['date_upd'] = time();
				$res = M("recent_updates")->add($data);
			}
			if ($res === false){
				$this->error('操作失败');
			}
			$this->success("操作成功",null,U('Setting/recent_updates'));
		}else {
			if ($update_id){
				$update = M("recent_updates")->where(['update_id' => $update_id])->find();
				
				$this->assign("updates",$update);
				$this->display();
			}else {
				$update = array(
						'update_id' => 0,
	...
	public function recent_updates_delete(){
		!($update_id = I("udpate_id")) && $this->error("请传入id");
		
		$res = M("recent_updates")->where(['update_id' => $update_id])->delete();
		if ($res === false){
			$this->error("操作失败");
		}
		$this->success("操作成功");
	...
	public function withdraw_fee(){
		if (IS_POST) {
			$withdraw_fee = I("withdraw_fee")/100;
			\app\library\SettingHelper::set("shuaibo_withdraw_fee",$withdraw_fee);
			$this->success("设置成功");
		}
		$withdraw_fee = \app\library\SettingHelper::get("shuaibo_withdraw_fee")*100;
		$this->assign("withdraw_fee",$withdraw_fee);
		$this->display();
	}
}