...
class Node extends Admin
{
	...
	public function edit(){
		...
		if(IS_POST) {
			...
			$res = D('node')->where(['id'=>(int)I('id')])->save($data);
				if($res !== false){
    					$this->success('添加成功！','',U('node/index'));
				}else{
					 $this->error('添加失败!');
	...
	{
		$nodes = D('node')->where(array('pid'=>$pid))->order('id ASC')->select();
		static $count = 0;
		foreach($nodes as $k=>$node){	
			$count++;
			$nodes[$k] = $node;
			$nodes[$k]['sort'] = $count;
			D('node')->where(array('id'=>$node['id']))->save(array('sort'=>$count));
