...
class Orders extends Admin
{
	...
	public function lists()
	{
		...
		->buildSql();
		$lists = M()->query("select o.*,os.name as state,c.uuid,concat(oa.province,oa.city,oa.district,oa.address) as address, ifnull(pt.name,'未支付') as pay_name, c.nickname,c.account,oa.name as address_name,oa.phone,ore.remind_order_id,co.amount as coupon_amount " .
        ...
        " LEFT JOIN {$prefix}order_info oi on oi.order_sn = o.out_order_sn" .
        " LEFT JOIN {$prefix}order_remind ore on ore.order_id = o.id " .
        " LEFT JOIN {$prefix}customer_coupon cco on cco.customer_coupon_id = o.coupon_id " .
        " LEFT JOIN {$prefix}coupon co on co.coupon_id = cco.coupon_id " .
		...
		if(!empty($ids)){
			...
			->join("goods g","g.goods_id = og.goods_id","LEFT")
                ...
                "GROUP_CONCAT(IFNULL(go.name,'默认')) as option_name,".
                    "GROUP_CONCAT(IFNULL(ore.state,0)) as refund_state,GROUP_CONCAT(IFNULL(ors.name,'无')) as refund_state_name,group_concat(og.address) as address")
			->group("og.order_id")
		...
		foreach ($lists as  &$l){
		    $pay_amount = $l['order_amount'] + $l['express_amount'] - $l['coupon_amount'] - $l['max_integration'] - $l['max_shopping_coin'];
		    $l["pay_amount"] = number_format($pay_amount,2,'.','');
            ...
            $address = explode(",",$order_goods[$l['id']][0]['address']);
            $goods = [];
		...
		->join("order o","o.order_state = os.order_state_id","LEFT")
	...
	public function details()
	{
		...
		->field("o.*,o.address as offaddress,c.nickname, c.avater, os.name as state,e.express_name,".
	...
	public function batch_carrier(){
		...
		->field("o.*,os.name as state,c.uuid,concat(oa.province,oa.city,oa.district,oa.address) as address, ifnull(pt.name,'未支付') as pay_name, c.nickname,c.account,oa.name as address_name,oa.phone,".
				"GROUP_CONCAT(g.name) as goods_names,GROUP_CONCAT( g.cover) as images,GROUP_CONCAT(og.price) as prices, GROUP_CONCAT(og.quantity) as quantitys,group_concat(og.address) as offline_address")
		->where($map)
		...
		foreach ($lists as &$l){
			...
			$address = explode(",",$l['offline_address']);
			$goods = [];
			for ($i = 0; $i< count($goods_name); $i++){
				$goods[] = [
						'goods_name' => $goods_name[$i],
	...
	public function excel(){
        ...
        $lists = M('order')
            ...
            ->join('customer c','o.customer_id=c.customer_id')
            ->join("order_address oa","oa.id = o.address_id")
            ->join("order_info oi","oi.order_sn = o.out_order_sn","LEFT")
		    ->field("o.order_sn,FROM_UNIXTIME(o.date_add) as date_add,ss.shop_name,GROUP_CONCAT( g.name, '×', og.quantity) as goods_names,".
                "c.nickname,oa.name as address_name,oa.phone as phone,concat(oa.province,oa.city,oa.district,oa.address) as address,".
                "o.order_amount,o.express_amount,o.max_shopping_coin,o.max_integration,IFNULL(oi.order_amount,'未支付') as pay_amount")
		    ->where($map)
		    ->group("o.id")
		    ->select();

        $filename="订单列表";
        $headArr=["订单编号","下单时间","店铺名称", "下单商品/数量",'会员账号','收货人','联系方式',"地址","订单金额","运费","购物积分", "积分", "支付金额"];
        $this->getExcel($filename,$headArr,$lists);
...
//
        ...
        $states = M("order_return_state")
            ...
            ->field("ors.order_return_state_id as id,count(orr.order_return_id) as count,ors.name")
            ->order("ors.order_return_state_id")
            ->select();


        $count = M("order_return")->count();
    ...
    public function refund(){
        ...
        if ($refund['type'] == 1) {
            ...
            if($state == 2){
                ...
                // 余额增加 积分会员积分退回
                $back = ['account' => ['exp', 'account + ' . $refund['price']], 'integration' => ['exp', 'integration + ' . $refund['max_integration']], 'shopping_coin' => ['exp', 'shopping_coin + ' . $refund['max_shopping_coin']],'hongfu' => ['exp','hongfu + '.$refund['max_hongfu']]];
                ...
                if ($refund['max_hongfu'] > 0){
                    ...
                    M("finance")->add(array(
                       ...
                       'type' => 4,
                        ...
                        'is_minus' => 2,
                        'order_sn' => $refund['refund_sn'],
                        'comments' => '退款订单'.$refund['refund_sn'],
...
//		$client->pushCache($message);
    ...
    public function mobile_list() {
        ...
        $map = " mr.type = 1 ";
        $map .=" AND mr.is_success = ". $state;
        $state == 0 && $map = " mr.type = 1 ";
        I('get.min_date') && $map .= " AND mr.date_add >= ".strtotime(I('get.min_date')) . " ";
        I("get.max_date") && $map .=" AND mr.date_add <= ".(strtotime(I("get.max_date")))  . " ";
        I('get.order_number') && $map .=" AND mr.order_sn like '".trim(I('get.order_number')) . "%' ";
        I('get.phone') && $map .=" AND mr.mobile like '".trim(I('get.phone')) . "%' ";
        ...
        $lists = M('mobile_recharge')
            ->alias('mr')
            ->where($map)
            ->join("customer c","c.customer_id = mr.customer_id")
            ->field("mr.recharge_id,mr.mobile,mr.order_sn,mr.amount,mr.total_fee,mr.type,mr.is_success,mr.date_add,".
                "c.nickname,c.customer_id")
            ->order("mr.date_add DESC")
            ->limit($page->firstRow . ',' . $page->listRows)
            ->select();
    ...
    public function confirm(){
        ...
        $this->log("订单号：{$order['order_sn']}后台确认收货成功");
        $this->success("操作成功");
    }
    ...
    public function closeorder() {
        ...
        $this->log("订单号：{$order['order_sn']}后台确认关闭成功");
        $this->success("操作成功");
    }
    ...
    public function recharge_list() {
        ...
        $map = " oi.order_type = 2 ";
        $map .=" AND oi.state = ". $state;
        $state == 0 && $map = " oi.order_type = 2 ";
        I('get.min_date') && $map .= " AND oi.date_add >= ".strtotime(I('get.min_date')) . " ";
        I("get.max_date") && $map .=" AND oi.date_add <= ".(strtotime(I("get.max_date")))  . " ";
        I('get.order_number') && $map .=" AND oi.order_sn like '".trim(I('get.order_number')) . "%' ";
        ...
        $lists = M('order_info')
            ->alias('oi')
            ->where($map)
            ->join("customer c","c.customer_id = oi.customer_id")
            ->field("oi.order_id,oi.order_sn,oi.pay_id,oi.pay_name,oi.state,oi.order_amount,oi.date_add,".
                "c.nickname,c.customer_id")
            ->order("oi.date_add DESC")
            ->limit($page->firstRow . ',' . $page->listRows)
            ->select();
