        ...
        if(self::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
                if(C('USER_AUTH_TYPE')==2) {
                    ...
                    //通过数据库进行访问检查
                    isset($_SESSION[C('USER_AUTH_KEY')]) ? $auth_key = $_SESSION[C('USER_AUTH_KEY')]:$auth_key='';
                    $accessList = self::getAccessList($auth_key);
                }else {
                    // 如果是管理员或者当前操作已经认证过，无需再次认证
                    if(isset($_SESSION[$accessGuid]) && $_SESSION[$accessGuid] !== false) {
                        return true;
                    }
                    //登录验证模式，比较登录后保存的权限访问列表
                    isset($_SESSION['_ACCESS_LIST']) ? $accessList = $_SESSION['_ACCESS_LIST'] : $accessList = '';
                }
                
                //判断是否为组件化模式，如果是，验证其全模块名
                if(!isset($accessList[strtoupper(CONTROLLER_NAME)][strtoupper(ACTION_NAME)])) {
 ...
 /*       $sql    =   "select node.id,node.name from ".
        ...
        foreach($apps as $key=>$app) {
            ...
            $modules =   $db->query($sql);*/
             $modules = M("role")->alias("r")
	            ->join("role_user u", "u.role_id = r.id")
	            ->join("access a","a.role_id=r.id  or (a.role_id=r.pid and r.pid!=0 )")
	            ->join("node n","a.node_id = n.id")
	            ->field("n.id, n.name")
	            ->where(['u.user_id' => $authId,'r.status' => 1,'n.level' => 2,'n.status'=>1])
	            ->select();
         //   print_r($modules);exit;
            ...
            foreach($modules as $key=>$module) {
                ...
                if('PUBLIC'== strtoupper($moduleName)) {
                	...
                	->join("node n","a.node_id = n.id")
                	->field("n.id, n.name")
                	->where(['u.user_id' => $authId,'r.status' => 1,'n.level' => 3,'n.status'=>1, 'n.pid' =>$moduleId])
                	->select();
            ...
            foreach($modules as $key=>$module) {
              ...
              //  $data[] = $moduleName;
                if(isset($access[strtoupper($moduleName)])){

                    $new[strtoupper($moduleName)]   =  array_change_key_case($action,CASE_UPPER);
                    $access = array_merge_recursive($access,$new); 
	...
	static public function getModuleAccessList($authId,$module) {
        ...
        ->join("access a","a.role_id=r.id  or (a.role_id=r.pid and r.pid!=0 )")
        ->field("a.node_id")
        ->where(['u.user_id' => $authId,'r.status' => 1,'a.status'=>1, 'a.module' =>$module])
        ->select();
