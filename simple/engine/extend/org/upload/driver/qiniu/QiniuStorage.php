...
class QiniuStorage
{
    ...
    public $QINIU_SOUTH_UP_HOST = "up-z2.qiniu.com";
    public $timeout        = '';
     ...
     * 
     * */
    public function getUploadToken($update = false){
    	//从缓存和数据库里找token
    	$token = \app\library\SettingHelper::get("leader_qiniu_token", ['deadline' => 0 , 'token' => '']);
    	//
    	if($update || empty($token) 
    			|| empty($token['token']) 
    			|| $token['deadline'] < time()){
    		$expires = 60 * 2;
    		$token = $this->UploadToken($this->sk, $this->ak, ['Expires' => $expires]);
    		
    		$token = ['deadline' => time() + $expires / 2, 'token' => $token];
    		\app\library\SettingHelper::set("leader_qiniu_token", $token);
    	}
    ...
    public function UploadToken($sk, $ak, $param)
    {
        ...
        if (isset($param['CallbackUrl']) && !empty($param['CallbackUrl'])) {
            $data['callbackUrl'] = $param['CallbackUrl'];
        }
        if (isset($param['CallbackBody']) && !empty($param['CallbackBody'])) {
            $data['callbackBody'] = $param['CallbackBody'];
        }
        if (isset($param['ReturnUrl']) && !empty($param['ReturnUrl'])) {
            $data['returnUrl'] = $param['ReturnUrl'];
        }
        if (isset($param['ReturnBody']) && !empty($param['ReturnBody'])) {
            $data['returnBody'] = $param['ReturnBody'];
        }
        if (isset($param['AsyncOps']) && !empty($param['AsyncOps'])) {
            $data['asyncOps'] = $param['AsyncOps'];
        }
        if (isset($param['EndUser']) && !empty($param['EndUser'])) {
    ...
    public function upload($config, $file)
    {
        ...
        if(isset($config['custom_fields']) && !empty($config['custom_fields']))
