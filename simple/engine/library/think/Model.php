...
class Model
{
    ...
    {   
        //APP_HOOK && Hook::listen('db_add',$_REQUEST);
        // 数据处理
    ...
    protected function _parseOptions($options = [])
    {
        ...
        if (isset($options['table'])) {
        	...
        	if(substr($options['table'],-6) == '_model'){
        		$options['table'] = substr($options['table'], 0,-6);
        	}
            $fields           = $this->getFields();
        }
    ...
    public function getAllRecords($tableName = ''){
    	...
    	$records = S("shuaibo_" . $tableName);
    	if(!APP_DEBUG && $records){
    		
    	}else{
    		$records = $this->select();
    		$records = serialize($records);
    	}
    	S("shuaibo_" . $tableName, $records);
     ...
     * @access public
     * @param string $tableName  数据表名 留空自动获取
     * @param bool $is_arr 取空数组
     * @return array
     */
    public function getFields($tableName = '', $is_arr = false)
    {
