...
class File
{
    ...
    public function rpush($name , $value){
    	$v = $this->get($name);
    	$v = empty($v) ? [] : unserialize($v);
    	$v[] = $value;
    	return $this->set($name, serialize($v));
    }
     ...
     * @param int $tail  结束
     */
    public function lranges($name,$head,$tail){
    	...
    	$last = $tail < 0 ? $length + $tail + 1 : $tail - $head + 1;
    	return array_slice($v, $head, $last);
    }
