...
class View
{
    ...
    private function parseTemplate($template)
    {
        ...
        if (defined('CONTROLLER_NAME')) {
            if ('' == $template) {
                // 如果模板文件名为空 按照默认规则定位
                $template = str_replace('.', DS, strtolower(CONTROLLER_NAME)) . $depr . ACTION_NAME;
            } elseif (false === strpos($template, $depr)) {
                $template = str_replace('.', DS, strtolower(CONTROLLER_NAME)) . $depr . $template;
            }
        }       
    ...
    private function getTemplateTheme()
    {
        if ($this->config['theme_on']) {
            if ($this->theme) {
                ...
                if (isset($_GET[$t])) {
                    $theme = $_GET[$t];
                } elseif (Cookie::get('vc_theme')) {
                    $theme = Cookie::get('vc_theme');
                }
                if (!isset($theme) || !is_dir($this->config['view_path'] . DS . $theme)) {
                    $theme = $this->config['default_theme'];
                }
                Cookie::set('vc_theme', $theme, 864000);
            } else {
                $theme = $this->config['default_theme'];
            }
            return $theme . DS;
