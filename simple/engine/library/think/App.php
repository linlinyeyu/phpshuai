...
class App
{
    ...
    public static function invokeMethod($method, $vars = [])
    {
        ...
        $site = \app\library\SettingHelper::get("shuaibo_close",['is_open' => 1 ,'reason' => '']);
        if(MODULE_NAME == "app" && $site['is_open'] == 0 && CONTROLLER_NAME != "weixin" && ACTION_NAME != "upload_files"){
        	return ['errcode' => 88 ,'message' =>'很抱歉，站点暂时关闭，原因为：' . $site['reason']];
        }
        
        $cache_name = MODULE_NAME . "." . CONTROLLER_NAME . "." .ACTION_NAME;
        $arr = C("SINGLE_THREAD");
        if(!empty($arr) && in_array($cache_name, $arr) && get_customer_id()){
        	$cache = Cache::get("shuaibo_". $cache_name . "_" . get_customer_id());
        	if(!empty($cache)){
        		return ['errcode' => '-101', 'message' => '正在处理中，请等待'];
        	}
        	Cache::set("shuaibo_". $cache_name . "_" . get_customer_id(), 1, 5);
        }
        $res = $reflect->invokeArgs(isset($class) ? $class : null, $args);
        Cache::rm("shuaibo_". $cache_name. "_" . get_customer_id() );
    ...
    {    
        ...
        if (APP_MULTI_MODULE) {
            ...
            // 获取模块名称
            !defined("MODULE_NAME") && define('MODULE_NAME', strip_tags($module));
            ...
            if (MODULE_NAME && !in_array(MODULE_NAME, $config['deny_module_list']) && is_dir(APP_PATH . MODULE_NAME)) {
                ...
                // 加载当前模块语言包
                if ($config['lang_switch_on'] && MODULE_NAME) {
                    Lang::load($path . 'lang' . DS . LANG_SET . DS . strtolower(CONTROLLER_NAME) . EXT);
                }
            } else {
            ...
            define('VIEW_PATH', MODULE_PATH . VIEW_LAYER . DS);
            // 加载当前模块语言包
            if ($config['lang_switch_on'] && CONTROLLER_NAME) {
                Lang::load(APP_PATH . 'lang' . DS . LANG_SET . DS . strtolower(CONTROLLER_NAME) . EXT);
            }
        }
