...
function att_show(serverData,file)
{
	...
	//附件aid
	var id = data.content.name;
	
	url = data.content.name;
	
	
	//附件地址
	var src = GV.IMG_URL + url;
	//附件类型
	var ext = 1;
	//附件名称
	var filename = url;
	...
	var div = $("<div/>").addClass("get").addClass("selected").attr("id","aid-" + id);
	$("<input type='hidden' />").attr("name","image_id[]").val(url).appendTo(div);
	$("<img  />").attr("src",src).attr("data-id",id).attr("data-path",src)
	.attr("title", filename).appendTo(div);

	$("<a/>").addClass("del btn1").html("删除").appendTo(div)
	.click(function(){
		$(this).parent().remove();
	});
...
{
	try {
		if($("#thumbnails div").length + numFilesSelected > 8){
			alert("图片最多选择8张");
			return;
		}
		this.startUpload();
