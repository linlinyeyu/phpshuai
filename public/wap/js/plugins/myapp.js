...
var app = angular.module("repeat-filter", []);
...
app.directive("imageResize", function(){
	return {
		link : function(scope, element, attrs){
			...
			if(attrs.avater){
				default_src = '/public/common/image/default-avater.png';
			}else if(attrs.goods){
				default_src= "/public/common/image/default-image-300.png";
			}else if(attrs.activity){
				default_src="/public/common/image/default-image-750.png";
			}
...
app.factory('LazyLoad',function($http){
	//初始化participate对象
 	...
 	participate.prototype.nextPage = function(){
 		...
 		//发送请求
 		this.currentAjax = pinet.get(this.url,data,function(response){
          		this.busy = false;
          		this.currentAjax = null;
...
app.directive('infinite', ['$rootScope', '$window', '$timeout','LazyLoad', function($rootScope, $window, $timeout, LazyLoad) {
       return {
         link: function(scope, elem, attrs) {
	    	 ...
	    	 if(attrs.bind){
	    		 if(scope[attrs.bind]){
	    			 lazyload.items = scope[attrs.bind];
	    			 console.log(lazyload.items);
	    		 }
...
var app = angular.module("emoji-filter", []);
app.filter('emoji',['$sce',function($sce){
	return function(input){
		...
		var c = {
	        ...
	        function initCss() {
	            var b = document.getElementsByTagName("head")[0] || document.createElement("head");
	            if (document.createStyleSheet) {
	                ...
	                c.type = "text/css";
	                c.innerHTML = ".RC_Expression {width:22px;height:22px;background-image:url(http://res.websdk.rongcloud.cn/css-sprite_bg.png);display:inline-block; vertical-align:top}";
	                b.appendChild(c)
	            }
	        }
	        var g = function(a) {
	        ...
	        var express =  b.Expression.retrievalEmoji(input, function(exp){
	        	var span = document.createElement("span");
                span.appendChild(exp.img);
                return "<span>" + span.innerHTML + "</span>";
	        });
	        
	        return $sce.trustAsHtml(express);
